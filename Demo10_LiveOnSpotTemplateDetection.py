...
while True:
    ...
    cv2.imshow("Take a photo of what you want to detect", frame)
    if not ret:
        break
    k = cv2.waitKey(1)
    ...
    if k%256 == 27:
        ...
        # SPACE pressed
        img_name = "LiveTemplate.png"
        input = frame[y0:y1, x0:x1]
        cv2.imwrite(img_name, input)
        print("{} written!".format(img_name))
        break

cap.release()
cv2.destroyAllWindows()
image_template = cv2.imread('LiveTemplate.png',0)
#image_template = cv2.resize(image_template, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_AREA)
cap = cv2.VideoCapture(0)


frame_width = int(cap.get(3))
frame_height = int(cap.get(4))
out = cv2.VideoWriter('Demo10.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (frame_width,frame_height))
